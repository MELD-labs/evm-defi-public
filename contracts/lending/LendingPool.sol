...
import {
    ...
    UserConfiguration
} from "../libraries/logic/ValidationLogic.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(address _addressesProvider) public initializer {
        ...
        maxStableRateBorrowSizePercent = 25_00; // 25%
        liquidationProtocolFeePercentage = 10_00; // 10%

        yieldBoostMultipliers[DataTypes.MeldBankerType.NONE][DataTypes.Action.DEPOSIT] = 100_00;
        yieldBoostMultipliers[DataTypes.MeldBankerType.BANKER][DataTypes.Action.DEPOSIT] = 107_00;
        yieldBoostMultipliers[DataTypes.MeldBankerType.GOLDEN][DataTypes.Action.DEPOSIT] = 120_00;

        yieldBoostMultipliers[DataTypes.MeldBankerType.BANKER][DataTypes.Action.BORROW] = 7_00;
        yieldBoostMultipliers[DataTypes.MeldBankerType.GOLDEN][DataTypes.Action.BORROW] = 20_00;

        emit LendingPoolInitialized(msg.sender, _addressesProvider);
    ...
    function deposit(
        ...
        uint256 _tokenId
    ) external override whenNotPaused nonReentrant returns (uint256 amountToDeposit) {
            ...
            require(
                !isUsingMeldBanker[_onBehalfOf] ||
                    (userMeldBankerData[_onBehalfOf].asset == _asset &&
                        ...
                        userMeldBankerData[_onBehalfOf].tokenId == _tokenId),
     ...
     *   - Send the value type(uint256).max in order to withdraw the whole mToken balance, within the constraints of the available liquidity
    ...
    function withdraw(
        ...
        if (_onBehalfOf != msg.sender) {
            // Only the Genius Loan role can withdraw on behalf of another user
            addressesProvider.checkRole(addressesProvider.GENIUS_LOAN_ROLE(), msg.sender);

            // check if user configuration has accepted genius loan
            require(
                usersConfig[_onBehalfOf].acceptGeniusLoan,
        ...
        returns (uint256 actualDebtToLiquidate, uint256 maxCollateralToLiquidate)
    {
        ...
        // Full collateral was used, and user does not hold any more tokens of this asset
        if (IERC20(reserves[_collateralAsset].mTokenAddress).balanceOf(_user) == 0) {
            YieldBoostLogic.MeldBankerData memory meldBankerNFTData = userMeldBankerData[_user];

            if (
                meldBankerNFTData.asset == _collateralAsset &&
                meldBankerNFTData.action == DataTypes.Action.DEPOSIT
            ) {
                YieldBoostLogic.unlockMeldBankerNFT(
                    isMeldBankerBlocked,
