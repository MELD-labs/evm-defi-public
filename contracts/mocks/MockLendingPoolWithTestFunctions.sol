...
/**
 * @title MockLendingPoolWithTestFunctions contract
 * @notice Implements the actions of the LendingPool, including deposit, borrow, repay, liquidation, etc
 * @dev Also includes an external mintToTreasury function for testing purposes because
 *   the MToken.mintToTreasury function can only be called by the LendingPool. Other test functions could be added
 *   in the future
 * @author MELD team
 */
contract MockLendingPoolWithTestFunctions is
    LendingBase,
    ILendingPool,
    IMeldBankerBlocked,
    UUPSUpgradeable,
    Initializable
{
    ...
    function setDoubleFlashLoanPremium(uint256 _flashLoanPremiumTotal) external {
        require(
            _flashLoanPremiumTotal <= PercentageMath.PERCENTAGE_FACTOR,
            Errors.VALUE_ABOVE_100_PERCENT
        );
        emit FlashLoanPremiumUpdated(msg.sender, flashLoanPremiumTotal, _flashLoanPremiumTotal);
        flashLoanPremiumTotal = 2 * _flashLoanPremiumTotal;
    }
     ...
     */
    function mintToTreasury(uint256 _amount, uint256 _index, address _reserve) external {
        DataTypes.ReserveData storage reserve = reserves[_reserve];
        address mToken = reserve.mTokenAddress;
        IMToken(mToken).mintToTreasury(_amount, _index);
    }
}
